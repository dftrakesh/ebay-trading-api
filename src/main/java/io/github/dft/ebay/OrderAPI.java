...
public class OrderAPI extends EbayTradingAPISdk {
    ...
    public GetSellerTransactionsResponse GetSellerTransactions(GetSellerTransactionsRequest getSellerTransactionsRequest) throws IOException, InterruptedException, URISyntaxException {
        getSellerTransactionsRequest.setRequesterCredentials(getEbayToken());
        String payload = toStr(getSellerTransactionsRequest);

        HttpRequest request = HttpRequest.newBuilder(new URI(XML_API_PRODUCTION_GATEWAY))
                .header(HTTP_HEADER_KEY_X_EBAY_API_CALL_NAME, EBAY_API_CALL_NAME_GET_SELLER_TRANSACTIONS)
                .header(HTTP_HEADER_KEY_X_EBAY_API_APP_NAME, requesterCredentials.getAppName())
                .header(HTTP_HEADER_KEY_X_EBAY_API_DEV_NAME, requesterCredentials.getDevName())
                .header(HTTP_HEADER_KEY_X_EBAY_API_CERT_NAME, requesterCredentials.getCertName())
                .header(HTTP_HEADER_KEY_X_EBAY_API_SITEID, requesterCredentials.getSiteID())
        ...
        HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
        String strResponse = response.body();
        return toObj(strResponse, GetSellerTransactionsResponse.class);
    }
}